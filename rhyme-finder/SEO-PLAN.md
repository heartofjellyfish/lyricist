...
Last updated: 2026-07-05 (pilot batch implemented — 100 pages live in
`rhyme-finder/rhymes/`, generated by `scripts/buildSeoPages.mjs`; run
`node scripts/buildSeoPages.mjs` to rebuild the pilot, `--full` for the
full derived batch once Search Console confirms indexing. Remaining:
attach Search Console, verify indexing for a week, then `--full`.)
